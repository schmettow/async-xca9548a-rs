...
#[doc(hidden)]
#[derive(Debug)]
pub struct Xca954xaData<'a, I2C> {
    /// The concrete I²C device implementation.
    pub(crate) i2c: I2C,
    /// The I²C device address.
    pub(crate) address: u8,
    pub(crate) selected_channel_mask: u8,
    _p: core::marker::PhantomData<&'a I2C>,
}

impl<'a, I2C, E> SelectChannels for Xca954xaData<'a, I2C>
where
    I2C: ehal::I2c<Error = E>,
    E: core::fmt::Debug,
{
    type Error = Error<E>;
    async fn select_channels(&mut self, channels: u8) -> Result<(), Self::Error> {
        self.i2c
...
macro_rules! i2c_traits {
    ( $name:ident ) => {
        impl<'a, I2C> DoOnAcquired<'a, I2C> for $name<'_, I2C> {
            async fn do_on_acquired<R, E: ehal::Error>(
                &self,
                f: impl AsyncFnOnce(cell::RefMut<Xca954xaData<I2C>>) -> Result<R, Error<E>>,
            ) -> Result<R, Error<E>> {
        ...
        impl<'a, I2C, E> ehal::ErrorType for $name<'a, I2C>
        where
            I2C: ehal::I2c<Error = E>,
            E: ehal::Error,
        {
        ...
        impl<'a, I2C, E> ehal::I2c for $name<'a, I2C>
        where
            I2C: ehal::I2c<Error = E>,
            E: ehal::Error,
        {
...
macro_rules! impl_device {
    ...
    ( $name:ident, $parts:ident, no_interrupts ) => {
            ...
            E: ehal::Error,
        {
            ...
            pub async fn get_channel_status(&self) -> Result<u8, Error<E>> {
                let mut data = [0];
                self.do_on_acquired(async |mut dev| {
            ...
            E: ehal::Error,
        {
            ...
            pub async fn select_channels(&mut self, channels: u8) -> Result<(), Error<E>> {
                self.do_on_acquired(async |mut dev| dev.select_channels(channels).await)
                    .await
            }
        }
    };
    ( $name:ident, $parts:ident, $mask:expr, interrupts ) => {
            ...
            E: ehal::Error,
        {
            ...
            pub async fn get_interrupt_status(&self) -> Result<u8, Error<E>> {
                let mut data = [0];
                self.do_on_acquired(async |mut dev| {
                    let address: u8 = dev.address;
                    dev.i2c
                        .read(address, &mut data)
                        .await
                        .map_err(Error::I2C)
            ...
            E: ehal::Error,
        {
            ...
            pub async fn select_channels(&mut self, channels: u8) -> Result<(), Error<E>> {
                self.do_on_acquired(async |mut dev| dev.select_channels(channels & $mask).await)
                    .await
            }
        }
    };
